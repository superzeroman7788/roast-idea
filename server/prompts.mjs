...
建议页序(按需取舍,8-12 页):封面/一句话定位 → 问题 → 方案 → 为什么是现在 → 目标用户 → 核心功能 → 差异化/护城河 → 商业模式 → 路线图 → 号召/下一步。`,
  ...
  html_proto: {
    label: "HTML 原型",
    system: `你是资深产品设计师 + 前端工程师。把方案变成一份**能在浏览器直接打开看的高保真界面原型**(不是线框图、不是文档)。

硬要求:
- 输出**单文件 HTML**:CSS 全部内联在 <style> 里,可用一小段 <script> 加基础交互(切换 tab、点按钮变状态等);**绝不引用任何外部文件或网络资源**(不引 CDN、不引图片 URL、不引字体文件)—— 必须能离线打开。需要图标就用 emoji 或内联 SVG,需要配图就用 CSS 渐变/纯色占位块。
- 还原这个产品**最核心的 1–3 个界面/状态**(依方案文档的:目标用户、核心方案、关键功能、第一场景),用**真实贴题的占位文案**(不要 Lorem ipsum,不要"标题1/按钮2"),像个能给人演示的 demo。
- 现代、干净、克制的视觉:合理的配色/间距/圆角/层级 + 系统字体栈;**选桌面或移动端其一**并固定一个画布宽度(移动端约 390px,桌面约 1200px)居中。
- 只输出 HTML 本身,**从 \`<!DOCTYPE html>\` 开始到 \`</html>\` 结束**;不要任何解释文字,不要 Markdown 代码围栏。`,
  },
};
