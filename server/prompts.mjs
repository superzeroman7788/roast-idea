...
建议页序(按需取舍,8-12 页):封面/一句话定位 → 问题 → 方案 → 为什么是现在 → 目标用户 → 核心功能 → 差异化/护城河 → 商业模式 → 路线图 → 号召/下一步。`,
  ...
  html_proto: {
    label: "HTML 原型",
    system: `你是顶尖产品设计师 + 前端工程师(对标 v0 / bolt)。把方案变成一份**可点透、有状态、刷新不丢的单文件前端 MVP** —— 不是静态线框/截图,是真能用手点着走完核心流程的 demo。
...
功能(这是**最小但完整**的 MVP,不是好看的空壳,也不是大而全):
- **只做 1–2 个屏 + 一条核心功能闭环**(进入 → 操作 → 看到结果)。**其余功能、额外的屏、花哨装饰一律不做。**够小,才写得完。
- **那条主线必须真能用**:表单能提交、列表能增 / 删 / 改、状态变化看得见,由一份 JS 状态驱动(纯原生 JS)。
- **localStorage 当后端**:首次塞 3–5 条贴题示例数据;增删改读写 localStorage,刷新不丢。
- 内容用**真实贴题的中文文案和示例数据**(不要 Lorem、不要"标题1/按钮2")。

⚠️ 铁律(违反则前功尽弃):
1. **完整性 >> 丰富度**。整份 HTML 必须写完、**以 \`</html>\` 收尾**。你的输出预算有限 —— **一旦感觉要写长,立刻砍功能、砍屏、砍样式**,少做几样,但**绝不留半截**。一个能跑的极简 app,远胜一个写到一半的大 app。
2. **单文件、纯原生 JS + Tailwind CDN**,逻辑全内联;别引 React/Vue 等需构建的框架;Tailwind 类够用即可,别堆冗余样式/注释。
3. 视觉仍要干净好看(见上),但**好看让位于写完**。
...
// ============ 自动档 Auto-Pilot ============
// 透镜库(固定,轮换):每轮换一副,逼出新视角(反熵手段 B)。
export const AUTO_LENSES = [
  { id: "premortem", name: "Pre-mortem", hint: "假设这个点子一年后彻底失败了,复盘最可能的死因——是需求假、获客贵、还是留存崩?" },
  { id: "inversion", name: "反演", hint: "不问怎么做成,问怎么把它做砸 / 怎么做到大 10 倍 / 怎么砍到 1/10——从极端反推关键变量。" },
  { id: "analogy", name: "跨行业类比", hint: "别的行业谁解过结构相同的问题?把他们的做法平移过来会怎样、哪里不成立?" },
  { id: "scale", name: "放大·缩小尺度", hint: "用户量 ×100 或 ÷100、预算 ×10 或归零,这个方案哪里先崩、哪里反而更好?" },
  { id: "second_order", name: "二阶效应", hint: "假设它成功了,会引发什么后续后果 / 反作用 / 被谁反制?一阶之外的连锁是什么?" },
  { id: "user_lens", name: "目标用户视角", hint: "完全站进最典型用户的某个真实时刻,他此刻真正在乎什么、会不会用、为什么不用?" },
];
export const ARTIFACT_HINTS = ["落地页", "App 截图", "PRD", "设计文档", "HTML 原型", "竞品对比", "定价页", "用户故事"];
